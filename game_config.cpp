...
std::map<sf::Keyboard::Key, Control> GameConfig::GetControls(int player_index) {
	...
	controls_[GetKey("TRADE_LOW" + ids)] = TRADE_LOW;
	controls_[GetKey("TRADE_HIGH" + ids)] = TRADE_HIGH;
	controls_[GetKey("DECLARE_WAR" + ids)] = DECLARE_WAR;
	controls_[GetKey("COMBAT_LOW" + ids)] = COMBAT_LOW;
	controls_[GetKey("COMBAT_HIGH" + ids)] = COMBAT_HIGH;
	return controls_;
}
